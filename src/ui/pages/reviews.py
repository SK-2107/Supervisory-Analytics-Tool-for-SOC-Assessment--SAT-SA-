...
def render_priority_cases_tab(conn, results, decision_map):
    ...
    # If tickets are not present (e.g. authentic public network telemetry dataset),
    # switch dynamically to Priority Security Alert Review Queue
    if df_tickets.empty:
        render_priority_alerts_review(conn, decision_map, name_lookup)
        return
...
def render_priority_alerts_review(conn, decision_map, name_lookup):
    """
    Renders the Priority Security Alert Review Queue when human tickets are not present.
    Allows supervisors to audit, verify, dismiss, or escalate authentic critical/high attack telemetry.
    """
    st.markdown(
        """
        <div class="satsa-card" style="border-left: 4px solid #087F73; background:#FAFCFC; margin-top: 10px; margin-bottom: 16px;">
            <div style="font-weight: 700; color: #172326; margin-bottom: 4px;">Public Telemetry Mode: Priority Incident Review Queue</div>
            <div class="card-note">
                Critical & High severity security telemetry events (NSL-KDD benchmark) are promoted into the <strong>Supervisory Incident Review Queue</strong> as active cases (<code>INC-2024-xxxxx</code>) for supervisory audit, validation, and disposition.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    df_alerts = conn.execute(
        """
        SELECT a.alert_id, a.cse_id, c.entity_name, c.sector, a.asset_id, a.timestamp,
               a.severity, a.source_system, a.rule_name, a.raw_summary
        FROM alerts a
        LEFT JOIN cses c ON a.cse_id = c.cse_id
        WHERE a.severity IN ('CRITICAL', 'HIGH')
        ORDER BY 
            CASE WHEN a.severity = 'CRITICAL' THEN 1 ELSE 2 END,
            a.alert_id ASC
        LIMIT 100
        """
    ).df()
    ...
    if df_alerts.empty:
        st.info("No Critical or High severity security incidents recorded in dataset.")
        return

    # Map raw alert ID to canonical Incident Case ID (e.g. ALT-NSL-00003 -> INC-2024-00003)
    df_alerts["case_id"] = df_alerts["alert_id"].apply(lambda aid: aid.replace("ALT-NSL-", "INC-2024-"))
    df_alerts["status"] = df_alerts.apply(
        lambda r: decision_map.get(r["case_id"], {}).get("status") or decision_map.get(r["alert_id"], {}).get("status", "OPEN"),
        axis=1
    )

    # Status KPI summary strip
    total_queue = len(df_alerts)
    open_cnt = len(df_alerts[df_alerts["status"] == "OPEN"])
    in_rev_cnt = len(df_alerts[df_alerts["status"] == "IN REVIEW"])
    conf_cnt = len(df_alerts[df_alerts["status"] == "CONFIRMED"])
    dism_cnt = len(df_alerts[df_alerts["status"] == "DISMISSED"])
    esc_cnt = len(df_alerts[df_alerts["status"] == "ESCALATED"])

    s_cols = st.columns(6)
    s_cols[0].markdown(f'<div class="satsa-kpi-block"><div class="satsa-kpi-label">Priority Incident Queue</div><div class="satsa-kpi-value">{total_queue}</div><div class="satsa-kpi-sub">Critical & High cases</div></div>', unsafe_allow_html=True)
    s_cols[1].markdown(f'<div class="satsa-kpi-block"><div class="satsa-kpi-label">Open</div><div class="satsa-kpi-value" style="color:#C58A18;">{open_cnt}</div><div class="satsa-kpi-sub">Awaiting review</div></div>', unsafe_allow_html=True)
    s_cols[2].markdown(f'<div class="satsa-kpi-block"><div class="satsa-kpi-label">In Review</div><div class="satsa-kpi-value" style="color:#2B6CB0;">{in_rev_cnt}</div><div class="satsa-kpi-sub">Under inspection</div></div>', unsafe_allow_html=True)
    s_cols[3].markdown(f'<div class="satsa-kpi-block"><div class="satsa-kpi-label">Confirmed</div><div class="satsa-kpi-value" style="color:#C93C3C;">{conf_cnt}</div><div class="satsa-kpi-sub">Threat confirmed</div></div>', unsafe_allow_html=True)
    s_cols[4].markdown(f'<div class="satsa-kpi-block"><div class="satsa-kpi-label">Dismissed</div><div class="satsa-kpi-value" style="color:#667579;">{dism_cnt}</div><div class="satsa-kpi-sub">Benign / Approved</div></div>', unsafe_allow_html=True)
    s_cols[5].markdown(f'<div class="satsa-kpi-block"><div class="satsa-kpi-label">Escalated</div><div class="satsa-kpi-value" style="color:#8B1E3F;">{esc_cnt}</div><div class="satsa-kpi-sub">Escalated to CSIRT</div></div>', unsafe_allow_html=True)

    st.write("")

    # Filters
    rf1, rf2, rf3 = st.columns([2.5, 2.5, 3])
    status_filter = rf1.selectbox("Filter Status", ["All Statuses", "OPEN", "IN REVIEW", "CONFIRMED", "DISMISSED", "ESCALATED"], key="rev_alt_status")
    sev_filter = rf2.selectbox("Filter Severity", ["All Severities", "CRITICAL", "HIGH"], key="rev_alt_sev")
    search_alert = rf3.text_input("Search Case ID / Asset", placeholder="INC-2024-… or AST-…", key="rev_alt_txt")
    ...
    if search_alert:
        filtered_alerts = filtered_alerts[
            filtered_alerts["case_id"].str.contains(search_alert, case=False, na=False) |
            filtered_alerts["alert_id"].str.contains(search_alert, case=False, na=False) |
            filtered_alerts["asset_id"].str.contains(search_alert, case=False, na=False)
        ]

    st.caption(f"Showing {len(filtered_alerts)} priority incident case(s)")

    active_alert_id = st.session_state.get("active_review_alert")

    # Incidents Table
    thead = st.columns([0.6, 1.6, 2.4, 1.2, 2.2, 1.8, 1.2, 1.2])
    for c, t in zip(thead, ["#", "Incident Case ID", "Target Entity", "Severity", "Attack Signature", "Asset Endpoint", "Status", "Action"]):
        c.markdown(f'<div class="tbl-head">{t}</div>', unsafe_allow_html=True)

    for idx, (_, row) in enumerate(filtered_alerts.head(50).iterrows()):
        aid = row["alert_id"]
        cid = row["case_id"]
        c_name = row["entity_name"]
        sev = row["severity"]
        rule = row["rule_name"]
        asset = row["asset_id"]
        curr_status = row["status"]
        ...
        r = st.columns([0.6, 1.6, 2.4, 1.2, 2.2, 1.8, 1.2, 1.2])
        r[0].markdown(f'<div class="{row_cls}">{idx + 1}</div>', unsafe_allow_html=True)
        r[1].markdown(f'<div class="{row_cls}"><strong>`{cid}`</strong></div>', unsafe_allow_html=True)
        r[2].markdown(f'<div class="{row_cls}">{c_name}</div>', unsafe_allow_html=True)
        r[3].markdown(f'<div class="{row_cls}">{badge(sev, sev)}</div>', unsafe_allow_html=True)
        r[4].markdown(f'<div class="{row_cls}">`{rule}`</div>', unsafe_allow_html=True)
        r[5].markdown(f'<div class="{row_cls}">{asset}</div>', unsafe_allow_html=True)
        r[6].markdown(f'<div class="{row_cls}"><span class="satsa-badge" style="background:#FAFCFC; color:{status_color}; border:1px solid {status_color}44;">{curr_status}</span></div>', unsafe_allow_html=True)

        if r[7].button("Inspect →", key=f"btn_insp_alt_{aid}", type="secondary", use_container_width=True):
            st.session_state["active_review_alert"] = aid
            st.rerun()

    # Active Incident Detail & Supervisory Decision Workspace
    if active_alert_id:
        st.markdown("<hr/>", unsafe_allow_html=True)
        render_alert_review_workspace(conn, active_alert_id, decision_map)


def render_alert_review_workspace(conn, alert_id, decision_map):
    """Renders supervisory audit workspace for an authentic incident case / security alert."""
    df_single = conn.execute(
        """
        SELECT a.alert_id, a.cse_id, c.entity_name, c.sector, a.asset_id, a.timestamp,
               a.severity, a.source_system, a.rule_name, a.raw_summary
        FROM alerts a
    ...
    rec = df_single.iloc[0].to_dict()
    case_id = alert_id.replace("ALT-NSL-", "INC-2024-")
    curr_dec = decision_map.get(case_id, {}) or decision_map.get(alert_id, {})
    current_status = curr_dec.get("status", "OPEN")
    current_notes = curr_dec.get("notes", "")

    st.markdown(f'<div class="section-title">Supervisory Audit Workspace: Incident Case `{case_id}`</div>', unsafe_allow_html=True)
    st.markdown('<div class="satsa-card" style="border: 1px solid #087F73; background:#FFFFFF; padding: 20px;">', unsafe_allow_html=True)

    c1, c2, c3, c4 = st.columns(4)
    c1.markdown(f"**Target Entity:**<br/>{rec['entity_name']}", unsafe_allow_html=True)
    c2.markdown(f"**Asset Endpoint:**<br/>`{rec['asset_id']}`", unsafe_allow_html=True)
    c3.markdown(f"**Incident Priority:**<br/>{badge(rec['severity'], rec['severity'])}", unsafe_allow_html=True)
    c4.markdown(f"**Trigger Alert ID:**<br/>`{rec['alert_id']}`", unsafe_allow_html=True)

    st.markdown("<hr/>", unsafe_allow_html=True)

    st.markdown("**Authentic Flow Telemetry Summary:**")
    st.code(rec["raw_summary"], language="text")

    st.markdown("**Observed Threat Signals:**")
    st.markdown(
        f"""
        <div class="explain-box">
            <div class="signal-check"><span class="signal-check-icon">✔</span><span>Signature Match: <strong>{rec['rule_name']}</strong></span></div>
            <div class="signal-check"><span class="signal-check-icon">✔</span><span>Telemetry Sensor: <strong>{rec['source_system']}</strong></span></div>
            <div class="signal-check"><span class="signal-check-icon">✔</span><span>Severity Level: <strong>{rec['severity']} Priority Attack Flow</strong></span></div>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    new_status = dec_cols[0].selectbox("Supervisory Action Status", status_options, index=def_idx, key=f"sel_status_alt_{case_id}")

    decision_reasons = {
        "CONFIRMED": "Confirm Threat Incident (Authentic intrusion verified)",
        "IN REVIEW": "Under Investigation (Packet analysis required)",
        "DISMISSED": "False Positive / Whitelisted (Authorized test flow)",
        "ESCALATED": "Escalate to CSIRT / Operational Response Team",
        "OPEN": "Pending Supervisory Review"
    }
    ...
    supervisor_notes = dec_cols[1].text_area(
        ...
        value=current_notes,
        placeholder="Record technical findings, packet inspection remarks, or containment instructions…",
        key=f"txt_notes_alt_{case_id}",
        height=85,
    )

    btn_c1, btn_c2, btn_c3 = st.columns([4, 1.5, 1.5])
    if btn_c2.button("Save Decision", key=f"btn_save_alt_{case_id}", type="primary", use_container_width=True):
        ...
        if success:
            st.success(f"✓ Supervisory decision recorded for {case_id}: {new_status}")
            st.rerun()
